...
const MODULOS_PORTAL_DEF = [
  ...
  {
    id: "importador-cartografia",
    titulo: "Importar Cartografía",
    descripcion: "Actualización controlada de predios SHP hacia PostGIS.",
    icono: "📥",
    roles: ["admin", "supervisor", "cartografia"],
...
function entrarModuloPortal(moduloId) {
  ...
  if (moduloId === "importador-cartografia") {
    document.body.classList.add("modo-portal-completo");
    restaurarTabsExtraPanel();
    if (typeof abrirModuloAdministracion === "function") {
      setTimeout(function() {
        abrirModuloAdministracion();
        if (typeof adminCambiarSeccion === "function") adminCambiarSeccion("cartografia");
      }, 80);
    }
  } else if (moduloId === "administracion") {
    document.body.classList.add("modo-portal-completo");
    restaurarTabsExtraPanel();
    if (typeof abrirModuloAdministracion === "function") {
